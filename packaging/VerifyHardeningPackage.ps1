...
$smokeRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("Icod.Terminal-hardening-package-smoke-{0}" -f [Guid]::NewGuid().ToString('N'))
New-Item -ItemType Directory -Path $smokeRoot -Force | Out-Null
try {
    Copy-Item -LiteralPath (Join-Path $repositoryRoot 'tools/package-hardening-smoke/Icod.Terminal.PackageHardeningSmoke.csproj') -Destination (Join-Path $smokeRoot 'Icod.Terminal.PackageHardeningSmoke.csproj')
    Copy-Item -LiteralPath (Join-Path $repositoryRoot 'tools/package-hardening-smoke/Program.cs') -Destination (Join-Path $smokeRoot 'Program.cs')
...
"@
    ...
    $project = Join-Path $smokeRoot 'Icod.Terminal.PackageHardeningSmoke.csproj'
    ...
    try {
        ...
        foreach ($framework in @('net8.0', 'net9.0', 'net10.0')) {
            Write-Host ''
            Write-Host "=== Fresh package 0.18 hardening consumer: $framework ==="
            Invoke-DotNet -Arguments @(
                'run',
                '--project', $project,
                '-c', $Configuration,
...
Write-Host "0.18 hardening package verification completed successfully for Icod.Terminal $ExpectedVersion ($Configuration)."
