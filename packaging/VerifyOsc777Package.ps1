...
$requiredMember = 'M:Icod.Terminal.TerminalSession.SendTitledNotificationAsync(System.String,System.String,System.Threading.CancellationToken)'
...
try {
    foreach ($framework in @('net8.0', 'net9.0', 'net10.0')) {
        ...
        if ($requiredMember -notin $documentedMembers) {
            throw "$entryPath is missing required 1.2 OSC 777 documentation '$requiredMember'."
        }
    }
} finally {
    $archive.Dispose()
...
$smokeRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("Icod.Terminal-osc777-package-smoke-{0}" -f [Guid]::NewGuid().ToString('N'))
New-Item -ItemType Directory -Path $smokeRoot -Force | Out-Null
try {
    Copy-Item -LiteralPath (Join-Path $repositoryRoot 'tools/package-osc777-smoke/Icod.Terminal.PackageOsc777Smoke.csproj') -Destination (Join-Path $smokeRoot 'Icod.Terminal.PackageOsc777Smoke.csproj')
    Copy-Item -LiteralPath (Join-Path $repositoryRoot 'tools/package-osc777-smoke/Program.cs') -Destination (Join-Path $smokeRoot 'Program.cs')
...
"@
    ...
    $project = Join-Path $smokeRoot 'Icod.Terminal.PackageOsc777Smoke.csproj'
    ...
    try {
        ...
        foreach ($framework in @('net8.0', 'net9.0', 'net10.0')) {
            Write-Host ''
            Write-Host "=== Fresh package OSC 777 consumer: $framework ==="
            Invoke-DotNet -Arguments @(
                'run',
                '--project', $project,
                '-c', $Configuration,
...
Write-Host "1.2 OSC 777 package verification completed successfully for Icod.Terminal $ExpectedVersion ($Configuration)."
