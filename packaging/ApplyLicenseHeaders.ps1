...
foreach ( $project in $projects ) {
	...
	$content = $content -replace '^<\?xml version="1\.0" encoding="utf-8"\?>\r?\n', ''
	if ( $content.Contains( 'Copyright (C) 2026  Timothy J. Bruce <uniblab@hotmail.com>' ) ) {
		throw "Project '$($project.FullName)' already contains the target copyright header; refusing to duplicate it."
	}

	$header = Get-ProjectHeader -AssemblyName $assemblyName -Description $description -LicenseKind $licenseKind
	$newContent = "<?xml version=`"1.0`" encoding=`"utf-8`"?>`n$header$content"
	[IO.File]::WriteAllText( $project.FullName, $newContent.Replace( "`r`n", "`n" ), $utf8NoBom )
}
...
foreach ( $source in $sources ) {
	...
	if ( $content.Contains( 'Copyright (C) 2026  Timothy J. Bruce <uniblab@hotmail.com>' ) ) {
		throw "Source '$($source.FullName)' already contains the target copyright header; refusing to duplicate it."
	}
	...
	$header = Get-CSharpHeader -AssemblyName $assemblyName -Description $description -LicenseKind $licenseKind
	$newContent = $header + $content.TrimStart( [char] 0xFEFF )
	[IO.File]::WriteAllText( $source.FullName, $newContent.Replace( "`r`n", "`n" ), $utf8NoBom )
}

Write-Host "Applied license headers to $($sources.Count) C# files and $($projects.Count) project files."
