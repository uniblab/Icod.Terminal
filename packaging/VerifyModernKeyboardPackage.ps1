...
$requiredMembers = @(
    'T:Icod.Terminal.TerminalKeyEventPhase',
    'T:Icod.Terminal.TerminalKeyboardReportingMode',
    'P:Icod.Terminal.TerminalInputEvent.KeyPhase',
    'P:Icod.Terminal.TerminalInputEvent.ShiftedCharacter',
    'P:Icod.Terminal.TerminalInputEvent.BaseLayoutCharacter',
    'P:Icod.Terminal.TerminalInputEvent.AssociatedText',
    'P:Icod.Terminal.TerminalInputProtocolOptions.KeyboardReportingMode',
    'P:Icod.Terminal.TerminalInputProtocolLease.KeyboardReportingMode'
)
...
try {
    foreach ($framework in @('net8.0', 'net9.0', 'net10.0')) {
        ...
        if (0 -ne $missingMembers.Count) {
            throw "$entryPath is missing required 0.17 modern-keyboard documentation: $($missingMembers -join ', ')."
        }
    }
} finally {
    $archive.Dispose()
...
$smokeRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("Icod.Terminal-modern-keyboard-package-smoke-{0}" -f [Guid]::NewGuid().ToString('N'))
New-Item -ItemType Directory -Path $smokeRoot -Force | Out-Null
try {
    Copy-Item -LiteralPath (Join-Path $repositoryRoot 'tools/package-modern-keyboard-smoke/Icod.Terminal.PackageModernKeyboardSmoke.csproj') -Destination (Join-Path $smokeRoot 'Icod.Terminal.PackageModernKeyboardSmoke.csproj')
    Copy-Item -LiteralPath (Join-Path $repositoryRoot 'tools/package-modern-keyboard-smoke/Program.cs') -Destination (Join-Path $smokeRoot 'Program.cs')
...
"@
    ...
    $project = Join-Path $smokeRoot 'Icod.Terminal.PackageModernKeyboardSmoke.csproj'
    ...
    try {
        ...
        foreach ($framework in @('net8.0', 'net9.0', 'net10.0')) {
            Write-Host ''
            Write-Host "=== Fresh package modern-keyboard consumer: $framework ==="
            Invoke-DotNet -Arguments @(
                'run',
                '--project', $project,
                '-c', $Configuration,
