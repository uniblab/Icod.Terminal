...
$requiredMembers = @(
    'M:Icod.Terminal.TerminalSession.SendNotificationAsync(System.String,System.Threading.CancellationToken)',
    'M:Icod.Terminal.TerminalSession.PublishWindowsCurrentDirectoryCompatibilityAsync(System.String,System.Threading.CancellationToken)'
)
...
try {
    foreach ($framework in @('net8.0', 'net9.0', 'net10.0')) {
        ...
        if (0 -ne $missingMembers.Count) {
            throw "$entryPath is missing required 0.16 safe OSC 9 documentation: $($missingMembers -join ', ')."
        }
    }
} finally {
    $archive.Dispose()
...
$smokeRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("Icod.Terminal-safe-osc9-package-smoke-{0}" -f [Guid]::NewGuid().ToString('N'))
New-Item -ItemType Directory -Path $smokeRoot -Force | Out-Null
try {
    Copy-Item -LiteralPath (Join-Path $repositoryRoot 'tools/package-safe-osc9-smoke/Icod.Terminal.PackageSafeOsc9Smoke.csproj') -Destination (Join-Path $smokeRoot 'Icod.Terminal.PackageSafeOsc9Smoke.csproj')
    Copy-Item -LiteralPath (Join-Path $repositoryRoot 'tools/package-safe-osc9-smoke/Program.cs') -Destination (Join-Path $smokeRoot 'Program.cs')
...
"@
    ...
    $project = Join-Path $smokeRoot 'Icod.Terminal.PackageSafeOsc9Smoke.csproj'
    ...
    try {
        ...
        foreach ($framework in @('net8.0', 'net9.0', 'net10.0')) {
            Write-Host ''
            Write-Host "=== Fresh package safe OSC 9 consumer: $framework ==="
            Invoke-DotNet -Arguments @(
                'run',
                '--project', $project,
                '-c', $Configuration,
...
Write-Host "0.16 safe OSC 9 package verification completed successfully for Icod.Terminal $ExpectedVersion ($Configuration)."
