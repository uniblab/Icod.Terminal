...
try {
    foreach ($framework in @('net8.0', 'net9.0', 'net10.0')) {
        ...
        if (0 -ne $missingMembers.Count) {
            throw "$entryPath is missing required 0.9 synchronized-output documentation: $($missingMembers -join ', ')."
        }
    }
} finally {
    $archive.Dispose()
...
$smokeRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("Icod.Terminal-synchronized-output-package-smoke-{0}" -f [Guid]::NewGuid().ToString('N'))
New-Item -ItemType Directory -Path $smokeRoot -Force | Out-Null
try {
    Copy-Item -LiteralPath (Join-Path $repositoryRoot 'tools/package-synchronized-output-smoke/Icod.Terminal.PackageSynchronizedOutputSmoke.csproj') -Destination (Join-Path $smokeRoot 'Icod.Terminal.PackageSynchronizedOutputSmoke.csproj')
    Copy-Item -LiteralPath (Join-Path $repositoryRoot 'tools/package-synchronized-output-smoke/Program.cs') -Destination (Join-Path $smokeRoot 'Program.cs')
...
"@
    ...
    $project = Join-Path $smokeRoot 'Icod.Terminal.PackageSynchronizedOutputSmoke.csproj'
    ...
    try {
        ...
        foreach ($framework in @('net8.0', 'net9.0', 'net10.0')) {
            Write-Host ''
            Write-Host "=== Fresh package synchronized-output consumer: $framework ==="
            Invoke-DotNet -Arguments @(
                'run',
                '--project', $project,
                '-c', $Configuration,
...
Write-Host "0.9 synchronized-output package verification completed successfully for Icod.Terminal $ExpectedVersion ($Configuration)."
