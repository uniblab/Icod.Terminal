...
$requiredMembers = @(
    'M:Icod.Terminal.TerminalSession.BeginVsCodePromptAsync(System.Threading.CancellationToken)',
    'M:Icod.Terminal.TerminalSession.BeginVsCodeCommandInputAsync(System.Threading.CancellationToken)',
    'M:Icod.Terminal.TerminalSession.BeginVsCodeCommandOutputAsync(System.Threading.CancellationToken)',
    'M:Icod.Terminal.TerminalSession.FinishVsCodeCommandAsync(System.Int32,System.Threading.CancellationToken)',
    'M:Icod.Terminal.TerminalSession.AbortVsCodeCommandAsync(System.Threading.CancellationToken)',
...
try {
    foreach ($framework in @('net8.0', 'net9.0', 'net10.0')) {
        ...
        if (0 -ne $missingMembers.Count) {
            throw "$entryPath is missing required 1.1 OSC 633 documentation: $($missingMembers -join ', ')."
        }
    }
} finally {
    $archive.Dispose()
...
$smokeRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("Icod.Terminal-osc633-package-smoke-{0}" -f [Guid]::NewGuid().ToString('N'))
New-Item -ItemType Directory -Path $smokeRoot -Force | Out-Null
try {
    Copy-Item -LiteralPath (Join-Path $repositoryRoot 'tools/package-osc633-smoke/Icod.Terminal.PackageOsc633Smoke.csproj') -Destination (Join-Path $smokeRoot 'Icod.Terminal.PackageOsc633Smoke.csproj')
    Copy-Item -LiteralPath (Join-Path $repositoryRoot 'tools/package-osc633-smoke/Program.cs') -Destination (Join-Path $smokeRoot 'Program.cs')
...
"@
    ...
    $project = Join-Path $smokeRoot 'Icod.Terminal.PackageOsc633Smoke.csproj'
    ...
    try {
        ...
        foreach ($framework in @('net8.0', 'net9.0', 'net10.0')) {
            Write-Host ''
            Write-Host "=== Fresh package OSC 633 consumer: $framework ==="
            Invoke-DotNet -Arguments @(
                'run',
                '--project', $project,
                '-c', $Configuration,
...
Write-Host "1.1 OSC 633 package verification completed successfully for Icod.Terminal $ExpectedVersion ($Configuration)."
