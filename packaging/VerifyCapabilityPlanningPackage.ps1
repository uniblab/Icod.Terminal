...
$requiredMembers = @(
	...
	'T:Icod.Terminal.TerminalCapabilityEvidenceKind',
	'T:Icod.Terminal.TerminalCapabilityStatus',
	'P:Icod.Terminal.TerminalCapabilityStatus.Capability',
	'P:Icod.Terminal.TerminalCapabilityStatus.Support',
	'P:Icod.Terminal.TerminalCapabilityStatus.EndpointAvailability',
	'P:Icod.Terminal.TerminalCapabilityStatus.EvidenceKind',
	'P:Icod.Terminal.TerminalCapabilityStatus.IsUsable',
	'M:Icod.Terminal.TerminalSession.InspectCapability(Icod.Terminal.TerminalCapability)',
	'M:Icod.Terminal.TerminalSession.VerifyCapabilityAsync(Icod.Terminal.TerminalCapability,System.Threading.CancellationToken)'
)
	...
	foreach ($framework in @('net8.0', 'net9.0', 'net10.0')) {
		...
		foreach ($requiredMember in $requiredMembers) {
			if ($requiredMember -notin $documentedMembers) {
				throw "$entryPath is missing required capability-planning documentation '$requiredMember'."
			}
		}
	}
} finally {
...
$smokeRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("Icod.Terminal-capability-planning-package-smoke-{0}" -f [Guid]::NewGuid().ToString('N'))
...
"@
	...
	$project = Join-Path $smokeRoot 'Icod.Terminal.PackageCapabilityPlanningSmoke.csproj'
	...
	try {
		...
		foreach ($framework in @('net8.0', 'net9.0', 'net10.0')) {
			Write-Host ''
			Write-Host "=== Fresh package capability-planning consumer: $framework ==="
			Invoke-DotNet -Arguments @(
				'run',
				'--project', $project,
				'-c', $Configuration,
