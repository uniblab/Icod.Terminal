...
/// <summary>
/// Freezes the T161 semantic capability and evidence boundary for persistent raster animation.
/// </summary>
public sealed class TerminalPersistentRasterAnimationCapabilityTests {
	[Fact]
	public void AnimationCapabilityUsesDistinctSemanticBackend() {
		...
		Assert.Equal( TerminalControlFamily.Apc, candidate.ControlFamily );
	}

	[Fact]
	public async Task AnimationInspectionEmitsNoTraffic() {
	...
	[Fact]
	public async Task VerifiedGenericKittyDoesNotVerifyPersistentRasterAnimation() {
		RecordingOutput output = new();
		await using TerminalSession session = await OpenSessionAsync(
			new RecordingTerminalControlProvider(),
			output
	...
	[Fact]
	public async Task VerifiedPlaceholderDoesNotVerifyPersistentRasterAnimation() {
		RecordingOutput output = new();
		await using TerminalSession session = await OpenSessionAsync(
			new RecordingTerminalControlProvider(),
			output
		...
		TerminalCapabilityStatus placeholder = session.InspectCapability(
		...
		Assert.Equal( TerminalCapabilitySupport.Verified, placeholder.Support );
		Assert.True( placeholder.IsUsable );
		Assert.Equal( TerminalCapabilitySupport.Unknown, animation.Support );
		Assert.Equal( TerminalCapabilityEvidenceKind.None, animation.EvidenceKind );
		Assert.False( animation.IsUsable );
		Assert.Empty( output.Bytes );
	...
	[Fact]
	public async Task AnimationVerificationDoesNotInventProbeTraffic() {
		RecordingOutput output = new();
		await using TerminalSession session = await OpenSessionAsync(
			new RecordingTerminalControlProvider(),
			output
		...
		Assert.Equal( TerminalCapabilitySupport.Unknown, before.Support );
		Assert.Equal( TerminalCapabilityEvidenceKind.None, before.EvidenceKind );
		Assert.Equal( before, after );
		Assert.Empty( output.Bytes );
	}

	[Fact]
	public async Task RedirectedOutputMakesAnimationCapabilityUnavailableWithoutTraffic() {
		RecordingOutput output = new();
		await using TerminalSession session = await OpenSessionAsync(
	...
	[Fact]
	public async Task AnimationProtocolEvidenceIsGenerationScoped() {
		RecordingOutput output = new();
		await using TerminalSession session = await OpenSessionAsync(
			new RecordingTerminalControlProvider(),
			output
		...
		Assert.True( verified.IsUsable );

		session.AdvanceSemanticLiveEvidenceGeneration();

		TerminalCapabilityStatus expired = session.InspectCapability(
			TerminalCapability.PersistentRasterAnimation
		);
		Assert.Equal( TerminalCapabilitySupport.Unknown, expired.Support );
		Assert.Equal( TerminalCapabilityEvidenceKind.None, expired.EvidenceKind );
