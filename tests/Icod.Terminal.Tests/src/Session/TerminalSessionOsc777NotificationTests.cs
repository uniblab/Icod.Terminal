...
/// <summary>
/// Verifies the public Icod.Terminal 1.2 OSC 777 titled-notification contract.
/// </summary>
public sealed class TerminalSessionOsc777NotificationTests {
	[Fact]
	public async Task EmitsExpectedTitledNotificationWithoutFlush() {
	...
	[Fact]
	public async Task InvalidFieldsFailBeforeWaitingForOutputGate() {
		RecordingTerminalOutput output = new();
		await using TerminalSession session = await OpenSessionAsync( output );
		...
		try {
			Assert.Throws<ArgumentException>(
				() => session.SendTitledNotificationAsync(
					"bad;title",
					"body"
				)
			);
	...
	[Fact]
	public async Task TitledNotificationWaitsBehindSharedOutputGate() {
		RecordingTerminalOutput output = new();
		await using TerminalSession session = await OpenSessionAsync( output );
		IDisposable controlOutput = await session.AcquireControlOutputAsync(
			CancellationToken.None
		...
		Task notification = session.SendTitledNotificationAsync(
			"Build",
			"Queued"
		).AsTask();
		await Task.Delay( 50 );
	...
	[Fact]
	public async Task LegacyAndTitledNotificationsRemainIndependent() {
		RecordingTerminalOutput output = new();
		await using TerminalSession session = await OpenSessionAsync( output );
	...
	private sealed class RecordingTerminalControlProvider : ITerminalControlProvider {
		...
		public TerminalControlResult<TerminalSize> GetSize(
			...
			return TerminalControlResult<TerminalSize>.Unsupported(
				"Size is not used by OSC 777 notification tests."
			);
		}
