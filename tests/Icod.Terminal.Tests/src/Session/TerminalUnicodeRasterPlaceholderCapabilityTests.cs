...
/// <summary>
/// Freezes the T151 semantic capability and evidence boundary for Unicode raster placeholders.
/// </summary>
public sealed class TerminalUnicodeRasterPlaceholderCapabilityTests {
	[Fact]
	public void PlaceholderCapabilityUsesDistinctSemanticBackend() {
	...
	[Fact]
	public async Task VerifiedGenericKittyDoesNotVerifyUnicodeRasterPlaceholders() {
		...
		Assert.True(
			TerminalCapabilityEndpointAvailability.Available
				== placeholder.EndpointAvailability
		);
		Assert.False( placeholder.IsUsable );
		Assert.Empty( output.Bytes );
	}

	[Fact]
	public async Task PlaceholderVerificationDoesNotInventProbeTraffic() {
		RecordingOutput output = new();
		await using TerminalSession session = await OpenSessionAsync(
			new RecordingTerminalControlProvider(),
			output
	...
	[Fact]
	public async Task RedirectedOutputMakesPlaceholderCapabilityUnavailableWithoutTraffic() {
		RecordingOutput output = new();
		await using TerminalSession session = await OpenSessionAsync(
