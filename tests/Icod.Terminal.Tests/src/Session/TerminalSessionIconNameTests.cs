...
/// <summary>
/// Verifies the T32 OSC 1 semantic icon-name operation without touching the host terminal.
/// </summary>
public sealed class TerminalSessionIconNameTests {
	[Theory]
	[InlineData( "", "1B5D313B1B5C" )]
	[InlineData( "icon", "1B5D313B69636F6E1B5C" )]
	public async Task SetIconNameEmitsExpectedOscOneFrame(
		string value,
		string expectedHex
	) {
		...
		await session.SetIconNameAsync( value );
	...
	[Fact]
	public async Task SetIconNameEncodesMultilingualTextAsUtf8() {
		RecordingTerminalOutput output = new();
		await using TerminalSession session = await OpenSessionAsync(
			new RecordingTerminalControlProvider(),
			output
		...
		await session.SetIconNameAsync( "éλ猫" );

		Assert.Equal(
			Convert.FromHexString( "1B5D313BC3A9CEBBE78CAB1B5C" ),
			output.Bytes.ToArray()
		);
	}

	[Fact]
	public async Task InvalidIconNameWritesNothing() {
		RecordingTerminalOutput output = new();
		await using TerminalSession session = await OpenSessionAsync(
			new RecordingTerminalControlProvider(),
			output
		...
		await Assert.ThrowsAsync<ArgumentException>(
			() => session.SetIconNameAsync( "bad\u001bicon" ).AsTask()
		);
