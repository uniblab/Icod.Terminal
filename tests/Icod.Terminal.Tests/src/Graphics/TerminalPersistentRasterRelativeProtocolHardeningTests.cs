...
/// <summary>
/// Defines T136 protocol-error and lifecycle hardening for relative persistent-raster placement.
/// </summary>
public sealed class TerminalPersistentRasterRelativeProtocolHardeningTests {
	[Fact]
	public async Task EnoparentRelativeUpdateInvalidatesParentSubtreeButKeepsResources() {
		...
		TerminalRasterResource descendantResource = await CreateResourceAsync(
			session,
			transport,
			imageId: 99,
			imageNumber: 3
		);
		TerminalRasterPlacement descendant = await CreateRelativePlacementAsync(
			descendantResource,
			child,
			transport,
			imageId: 99,
		...
		Assert.Equal(
			...
			( await parent.UpdateAsync() ).Status
		);
		Assert.Equal(
			TerminalControlStatus.Unavailable,
			( await child.UpdateRelativeAsync( 0, 0 ) ).Status
		);
		Assert.Equal(
			TerminalControlStatus.Unavailable,
			( await descendant.UpdateRelativeAsync( 0, 0 ) ).Status
		);
		Assert.Equal( baselineWrites, transport.Writes.Count );

		Task<TerminalControlResult<TerminalRasterPlacement>> replacement =
			parentResource.CreatePlacementAsync().AsTask();
		await transport.WaitForWriteCountAsync( baselineWrites + 1 );
		transport.Publish(
			Encoding.ASCII.GetBytes( "\u001b_Gi=77,p=4;OK\u001b\\" )
		);
		Assert.Equal(
			TerminalControlStatus.Available,
			( await replacement ).Status
		);
	}

	[Fact]
	public async Task EnoparentRelativeCreationInvalidatesParentSubtreeButKeepsChildResource() {
		ScriptedTransport transport = new();
		await using TerminalSession session = await OpenSessionAsync( transport );
		TerminalRasterResource parentResource = await CreateResourceAsync(
			session,
		...
		Task<TerminalControlResult<TerminalRasterPlacement>> replacement =
			childResource.CreatePlacementAsync().AsTask();
		await transport.WaitForWriteCountAsync( baselineWrites + 1 );
		transport.Publish(
	...
	[Theory]
	[InlineData( "ECYCLE:synthetic cycle" )]
	[InlineData( "ETOODEEP:synthetic excessive depth" )]
	public async Task StructuralProtocolFailuresDoNotInvalidateRelativeGraph(
		string failureMessage
	) {
		ScriptedTransport transport = new();
		await using TerminalSession session = await OpenSessionAsync( transport );
		(
			TerminalRasterPlacement _,
			TerminalRasterPlacement child
		) = await CreateRelativePairAsync( session, transport );
		int expectedWriteCount = transport.Writes.Count + 1;

		Task<TerminalControlMutationResult> failedUpdate = child.UpdateRelativeAsync(
			columnOffset: 7,
			rowOffset: -8
		).AsTask();
		await transport.WaitForWriteCountAsync( expectedWriteCount );
		...
		Assert.Equal( 9, child.State.ColumnOffset );
		Assert.Equal( 10, child.State.RowOffset );
	}

	[Fact]
	public async Task EnoentRelativeUpdateInvalidatesChildResourceWithoutInvalidatingParentResource() {
		ScriptedTransport transport = new();
		await using TerminalSession session = await OpenSessionAsync( transport );
		TerminalRasterResource parentResource = await CreateResourceAsync(
			session,
		...
		Assert.Equal(
			TerminalControlStatus.Unavailable,
			( await childResource.CreatePlacementAsync() ).Status
		);
		Assert.Equal( baselineWrites, transport.Writes.Count );

		Task<TerminalControlResult<TerminalRasterPlacement>> parentReplacement =
			parentResource.CreatePlacementAsync().AsTask();
		await transport.WaitForWriteCountAsync( baselineWrites + 1 );
		transport.Publish(
			Encoding.ASCII.GetBytes( "\u001b_Gi=77,p=3;OK\u001b\\" )
		);
		Assert.Equal(
			TerminalControlStatus.Available,
			( await parentReplacement ).Status
		);
	}
