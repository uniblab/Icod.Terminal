...
/// <summary>
/// Verifies advanced placement geometry through the public persistent-raster lifecycle.
/// </summary>
public sealed class TerminalPersistentRasterAdvancedPlacementIntegrationTests {
	[Fact]
	public async Task CreateAndUpdateCarryAdvancedGeometryWithStablePlacementIdentity() {
			...
			await using ( placement ) {
				Assert.Equal(
					Encoding.ASCII.GetBytes(
						"\u001b_Ga=p,i=77,p=1,C=1,x=1,y=2,w=4,h=5,c=6,r=7,z=-3\u001b\\"
					),
					transport.Writes[ 1 ]
				);

				TerminalRasterPlacementOptions updateOptions = new() {
					SourceRectangle = new TerminalRasterSourceRectangle(
						2,
						1,
						3,
				...
				Assert.Equal(
					Encoding.ASCII.GetBytes(
						"\u001b_Ga=p,i=77,p=1,C=1,x=2,y=1,w=3,h=4,c=5,r=4,z=-2147483648\u001b\\"
					),
					transport.Writes[ 2 ]
				);
			}
