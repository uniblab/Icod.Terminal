...
- each target-framework dependency group contains exactly one `Icod.TermInfo` and one `Icod.Timing` runtime dependency entry;
- dependency version compatibility is not duplicated as a verifier contract: the package project declares its dependency requirements, while successful NuGet restore/build provides compatibility evidence;
...
The verifier deliberately does **not** hard-code expected `Icod.TermInfo` or `Icod.Timing` version strings. Exact dependency-version assertions would duplicate project metadata and turn release validation into an unnecessary coupling point.
