# Icod.Terminal Public API Baseline — 1.0.0-rc1

**Release:** `1.0.0-rc1`  
**Captured from:** exact head `510f9738f216064f80193eba1936583b5552170e`  
**Capture validation:** workflow #926  
**Target frameworks:** `net8.0`, `net9.0`, `net10.0`

## Purpose

This document freezes the public source/binary contract that `Icod.Terminal` intends to carry into the 1.x line.
...
All three supported TFMs produced byte-identical logical snapshots during workflow #926.

After normalizing line endings to LF, the frozen SHA-256 is:
...
`packaging/VerifyPublicApiBaseline.ps1` regenerates the snapshot independently for net8.0, net9.0, and net10.0, first proves the TFMs agree with one another, and then proves the normalized fingerprint matches this frozen value.
...
This is the only intentional public removal between the published 0.18 surface and the rc1 baseline.
...
After the 1.0 contract is accepted, accidental removal, renaming, signature drift, enum renumbering, or TFM-specific public-surface divergence is a release defect.
...
The rc1 fingerprint is therefore both a release-candidate artifact and the starting compatibility authority for the 1.x line.
